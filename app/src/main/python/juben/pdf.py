...
def get_font(font=''):
    # 从环境变量中获取由安卓端传递过来的字体目录路径
    fonts_path = os.environ.get('INJUBEN_FONTS_PATH')
    if fonts_path:
        return os.path.join(fonts_path, font)
    else:
        # 作为备用方案，这在安卓环境中会失败，但在本地测试时有用
        return os.path.join(os.path.dirname(os.path.abspath(__file__)), 'fonts', font)

registerFont(TTFont('source-han-serif', get_font('SourceHanSerifSC-Light.ttf')))
registerFont(TTFont('source-han-serif-bold', get_font('SourceHanSansSC-Medium.ttf')))
registerFont(TTFont('source-han-sans', get_font('SourceHanSansSC-Regular.ttf')))
registerFont(TTFont('source-han-sans-bold', get_font('SourceHanSansSC-Medium.ttf')))
