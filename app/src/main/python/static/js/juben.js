$(function() {
    ...
    // We will load content from files, so this is no longer needed.
 ...
 var in_editor = CodeMirror.fromTextArea(in_text, {
    ...
    in_editor.on('change', function(cm) {
        ...
        save_timer = setTimeout(save_current_file, 1500);
    });
    ...
    $(document).keydown(function(event) {
        if((event.ctrlKey || event.metaKey) && event.which == 83) {
            ...
            // save_in_to_local_storage(); // Replaced with file saving
    ...
    $('#in-save-txt').on('click', function() {
        ...
        var finalFileName = file_name + suffix + ".txt";

        if (window.Android && typeof window.Android.downloadTxt === 'function') {
            // If the Android interface exists, use it
            window.Android.downloadTxt(content, finalFileName);
        } else {
            // Fallback for web browsers
            if(window.navigator.userAgent.match(/Win/)) content = content.replace(/\n/g, "\r\n");
            download(base64_to_blob(window.btoa(unescape(encodeURIComponent("\ufeff" + content))), 'text/plain;charset=utf-8'), finalFileName, "text/plain;charset=utf-8");
        }
    });
    ...
    $('#in-save-pdf').on('click', function(e) {
        if($(this).attr('disabled')) return false;
        var fileName = injuben_result.filename + '.' + injuben_result.suffix;
        if (window.Android && typeof window.Android.downloadPdf === 'function') {
            // If the Android interface exists, use it
            window.Android.downloadPdf(injuben_result.content, fileName);
        } else {
            // Fallback for web browsers
    ...
    load_editor_option();
    load_preview_option();
    // load_state(); // We will manage state differently now
            ...
            success: function(files) {
                ...
                if (files.length > 0) {
                    files.forEach(function(file) {
                        var li = $('<li>').attr('data-filename', file);
                        var fileNameSpan = $('<span>').addClass('file-name').text(file);
                        var renameBtn = $('<button>').addClass('button is-small is-info rename-btn').attr('title', '重命名').html('<i class="fi-pencil"></i>');
                        var deleteBtn = $('<button>').addClass('button is-small is-danger delete-btn').attr('title', '删除').html('&times;');

                        li.append(fileNameSpan).append(renameBtn).append(deleteBtn);
                        file_list.append(li);
                    });
    ...
    function load_file(filename) {
        ...
        $.ajax({
            ...
            success: function(data) {
                in_editor.setValue(data.content);
                current_file = filename;
                $('#file-list li').removeClass('is-active');
                $('#file-list li[data-filename="' + filename + '"]').addClass('is-active');
                submit_preview();
            },
            error: function(xhr) {
                alert("Error loading file: " + xhr.responseText);
            }
        });
    }
    ...
    function save_current_file() {
        ...
        var content = in_editor.getValue();
        $.ajax({
            url: '/api/files/' + current_file,
            type: 'POST',
    ...
    function create_new_file() {
        var filename = "未命名_" + new Date().toISOString().slice(0, 19).replace('T', '_').replace(/:/g, '-') + ".fountain";
        $.ajax({
            url: '/api/files/' + filename,
            type: 'POST',
            contentType: 'application/json',
            data: JSON.stringify({ content: '标题: ' + filename.replace('.fountain', '') + '\n\n' }),
            success: function() {
                load_file_list();
            },
            error: function(xhr) {
                alert("Error creating file: " + xhr.responseText);
            }
        });
    }
    
    $('#file-list').on('click', 'li .file-name', function() {
        var filename = $(this).parent().data('filename');
        load_file(filename);
    });

    // Rename file
    $('#file-list').on('click', '.rename-btn', function(e) {
        e.stopPropagation();
        var li = $(this).parent();
        var fileNameSpan = li.find('.file-name');
        var old_filename = li.data('filename');
        var input = $('<input type="text" class="file-rename-input">').val(old_filename);
        
        fileNameSpan.replaceWith(input);
        input.focus();

        function saveRename() {
            var new_filename = input.val();
            if (new_filename && new_filename !== old_filename) {
                rename_file(old_filename, new_filename);
            } else {
                input.replaceWith(fileNameSpan); // Restore original if no change
            }
        }
    ...
    function rename_file(old_filename, new_filename) {
        $.ajax({
            url: '/api/files/' + old_filename,
            type: 'PUT',
            contentType: 'application/json',
            data: JSON.stringify({ new_filename: new_filename }),
            success: function() {
                load_file_list();
            },
            error: function(xhr) {
    ...
    $('#new-file-btn').on('click', function() {
        create_new_file();
        $('#file-list-modal').removeClass('is-active');
    });

    $('#floating-file-btn').on('click', function() {
        $('#file-list-modal').addClass('is-active');
    });

    $('#file-list-modal .modal-background, #file-list-modal .delete').on('click', function() {
        $('#file-list-modal').removeClass('is-active');
    });
    ...
    function save_state() {
        // save_in_to_local_storage(); // Replaced
        if(current_file) save_current_file();
    ...
    // Expose functions to be called from Android
    window.getEditorText = function() {
        return in_editor.getValue();
    };

    window.setEditorText = function(text) {
        in_editor.setValue(text);
    };
});
